...
Complete schema and configuration reference for `<skill-path>/skill-rules.json`.
...
**Path:** `<skill-path>/skill-rules.json`

This JSON file defines skill trigger conditions for the auto-activation system, where `<skill-path>` is the directory that contains `SKILL.md`.
...
```bash
cat <skill-path>/skill-rules.json | jq .
```
...
**Related Files:**
- [SKILL.md](SKILL.md) - <skill-path>/SKILL.md
- [tigger-types.md](tigger-types.md) - Complete trigger documentation
